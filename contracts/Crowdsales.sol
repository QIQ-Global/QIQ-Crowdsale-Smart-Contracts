...
contract QIQTokenCrowdSale is OwnedByICOOwner {
    ...
    // ==== address of QIQ Token ==== //
    QIQToken public token;
    ...
    // ==== hardcap of ICO ==== //
    uint256 public hardCap;             // hardcap value
    ...
    function QIQTokennCrowdSale(address _QIQToken, address _contributionColdStorage,
                                uint256 _hardCap,
                                uint256 _startDate, uint256 _endDate) public {
        token = QIQToken(_QIQToken);             // initialising reference to ERC20
    ...
    function calculateBonuses(uint256 amount) internal view returns (uint256 total) {
        ...
        // therefore 1 QTC = 0.00022 ETH
        uint256 x = amount * 10 ** 18;
        return x.div((220000000000000));
    }
